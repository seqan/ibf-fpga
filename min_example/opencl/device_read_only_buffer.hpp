...
struct device_read_only_buffer
{
    ...
    device_read_only_buffer(std::string_view buffer_name, value_type * host_ptr, size_t const host_size, cl::Context & context) :
        buffer_name{buffer_name},
        host_ptr{(void *)host_ptr},
        host_size{host_size},
        host_size_bytes{host_size * sizeof(value_type)},
        cl_device_buffer{context, CL_MEM_READ_ONLY, host_size_bytes}
    {}
    ...
    void transfer2device(cl::CommandQueue & utilitiesQueue)
    {
        std::cerr << "Transfer " << buffer_name << ": " << host_size_bytes << " bytes" << std::endl;
        utilitiesQueue.enqueueWriteBuffer(
